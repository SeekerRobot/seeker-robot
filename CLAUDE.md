...
- **`ros2_ws/`** — ROS 2 colcon workspace. Packages live in `ros2_ws/src/`:
  ...
  - `seeker_navigation` — Nav2, SLAM Toolbox, EKF configs. Mission planner split across `ball_searcher` and `object_seeker.py` — a YOLO-driven WANDER/SEEK/PERFORM_MOVE state machine exposed as the `SeekObject` Action Server. `find.py` is a CLI client for manual goals. Additional utility nodes: `scan_tilt_filter` (drops LiDAR rays corrupted by body tilt), `dead_reckoning_odom` (open-loop cmd_vel integration for no-gyro testing), `cmd_vel_restrict` (enforces no-strafing and angular-wins-over-linear), `scripted_cmd_vel` (YAML-scripted cmd_vel replay). Launch files: `real_slam_raw`, `real_slam_ekf`, `real_ball_search`, `real_object_seek` (EKF + SLAM + Nav2 + YOLO + object_seeker), `real_object_seek_no_gyro` (same but dead-reckoning), `real_scripted_drive` (scripted cmd_vel + SLAM).
...
- **Board environments**: `esp32s3sense` (Seeed XIAO ESP32-S3, default), `esp32dev` (generic ESP32-WROOM-32), and `esp32cam` (AI-Thinker ESP32-CAM). Serial transport variants: `esp32devserial`, `esp32s3senseserial`. OTA upload variants: `esp32dev_ota`, `esp32s3sense_ota`. Bare (no micro-ROS) variants: `esp32dev_bare`, `esp32s3sense_bare`. Pin definitions are in `mcu_ws/lib/RobotConfig/RobotConfig.h`, gated by `ENV_ESP32S3SENSE` / `ENV_ESP32DEV` / `ENV_ESP32CAM` macros set by the board's build flags.
