...
| OLED display dead / wrong frame | `test_sub_oled` (serial-only draw), then `test_bridge_oled` (HTTP LCD path from host :8384), fall back to `test_raw_oled` to rule out `OledSubsystem` |
